...
// ベクトルを正規化する
void Vector2Normalize(Vector2* pOut, const Vector2& vec0)
// 2Dベクトル内積
...
// ベクトルの長さを取得
void Vector3Length(float* pOut, const Vector3& vec)
// ベクトルを正規化する
...
// 0.0 ~ 1.0の間にクランプする
float clamp0to1(const float& val)

// 指定した値の間にラップする
template<typename T>
T wrap(const T& min, const T& max, const T& val)

// PIにラップする
float wrapPI(const float& rad)
double wrapPI(const double& rad)
...
//*****************************************************************************
// 円周率
static const float KB_PI = 3.141592653589f;
// 度数法の1度
static const float ONE_DEGREE = KB_PI / 180.0f;
// 2の平方根の逆数
static const float ONE_OVER_RT2 = 0.7071067811865475f;
...
// ベクトルの長さを取得
// Author : Ryo Kobayashi
// LastUpdate : 2014/6/29
//=============================================================================
inline float Vector2Length(const Vector2& vec)
{
	return sqrtFast(vec.x * vec.x + vec.y * vec.y);
}
...
// LastUpdate : 2014/6/29
//=============================================================================
inline void Vector2Normalize(Vector2* pOut, const Vector2& vec0)
{
	float fMag = sqrtFast(vec0.x * vec0.x + vec0.y * vec0.y);
...
//=============================================================================
inline void Vector3Length(float* pOut, const Vector3& vec)
{
	*pOut = sqrtFast(vec.x * vec.x + vec.y * vec.y + vec.z * vec.z);
}
...
// LastUpdate : 2014/7/8
//=============================================================================
inline float clamp0to1(const float& val)
{
	float ret;
	if(val < 0.0f)
	{
...
//=============================================================================
template<typename T>
inline T wrap(const T& min, const T& max, const T& val)
{
	T ret = val;
	T diff = max - min;
...
//=============================================================================
// PIを -pi ~ pi にラップする (float)
// Author : Ryo Kobayashi
// LastUpdate : 2014/7/8
//=============================================================================
inline float wrapPI(const float& rad)
{
	float ret = rad;
