...
void CNetworkDataBuffer::PushBullet(NETWORK_DATA* pData)
{
	...
	{
		case NETWORK_DATA_TYPE_POSITION:
			m_BulletInfoBuffer[pData->my_ID].player_id = pData->my_ID;
			//m_CharcterInfoBuffer[pData->my_ID].end_push_flag = false;
			...
			m_ReciveStateBullet[pData->my_ID].recive_position = true;
		break;
		...
		case NETWORK_DATA_TYPE_FRONT_VECTOR:
			...
			m_ReciveStateBullet[pData->my_ID].recive_rotation = true;
		break;
		...
		case NETWORK_DATA_TYPE_SPEED:
			...
			m_ReciveStateBullet[pData->my_ID].recive_speed = true;
		break;
		...
		case NETWORK_DATA_TYPE_ALL_SEND:
			...
			// ちゃんと詰まってるなら
			if(m_ReciveStateBullet[pData->my_ID].recive_speed == true
				&& m_ReciveStateBullet[pData->my_ID].recive_position == true
				&& m_ReciveStateBullet[pData->my_ID].recive_rotation == true)
			{
				m_BulletInfoBuffer[pData->my_ID].end_push_flag = true;
			}

			// フラグ落とす
			m_ReciveStateBullet[pData->my_ID].recive_speed = false;
			m_ReciveStateBullet[pData->my_ID].recive_position = false;
			m_ReciveStateBullet[pData->my_ID].recive_rotation = false;
		break;
